          ...
          char *argv[] )
{
  ...
  GtkWidget *valign;
   ...
   //  gtk_box_pack_start (GTK_BOX (GTK_DIALOG(window)->hbox), scrolled_window, 
			// TRUE, TRUE, 0);
    gtk_widget_show (scrolled_window);
    
    /* create a table of 10 by 10 squares. */
    table = gtk_table_new (12, 10, FALSE);
   ...
   //                                (char*)data ,
   //                                strlen((char*)data));
   gtk_table_attach(GTK_TABLE(table), wins, 0, 7, i*2, 2*i+2, 
   ...
   gtk_widget_show (wins);
    ...
    for (i = 0; i < 10; i++){
	  ...
	  gtk_table_attach(GTK_TABLE(table), button, 8,9,i, i+1, GTK_FILL | GTK_SHRINK, GTK_FILL | GTK_SHRINK, 0, 0);
